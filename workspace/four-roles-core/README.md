...
- `roles/pm.md`, `architect.md`, `dev.md`, `qa.md` — 四 Agent 说明与输出要求
...
- **`runs/`** — 运行日志目录，每次四 Agent 协作生成一份 `<runId>.json`，便于测试与回溯
...
- **在 OpenClaw / 飞书**：用户说「按四 Agent 流程做 XXX」或「四 Agent 协作」时，Agent 加载 skill-four-roles，按本目录的 flow 与 roles 推进，读写 context.json。
