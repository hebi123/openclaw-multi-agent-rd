...
当前已改为 **4 个 Agent 协作**：main（编排者）+ pm、architect、dev、qa（四 Agent）。飞书等入口仍由 main 接收，main 通过 **sessions_send** 将各阶段任务发给对应 Agent，收集回复后更新 context 并推进流程。
...
- **openclaw.json**（本地，已加以下内容，因 gitignore 未提交）：
  ...
  - **main.subagents.allowAgents**：["pm", "architect", "dev", "qa"]，允许 main 向四 Agent 发消息。
...
- 在飞书问：「你现在是多 Agent 还是单 Agent？」  
  应回答：当前为**多 Agent**，有 main、pm、architect、dev、qa 五个 Agent；四 Agent 协同时由 main 通过 sessions_send 调用 pm/architect/dev/qa。
- 说「按四 Agent 流程做 XXX」：main 应按流程向 agent:pm:main、agent:architect:main 等发消息并收集产出。
