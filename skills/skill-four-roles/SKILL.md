...
description: 四 Agent 协作（产品经理、技术架构师、开发经理、测试总监）按既定流程协作。当用户要求「按四 Agent 流程」「四 Agent 协作」「启动四 Agent」或对需求进行完整的需求→方案→排期→可测性→四方对齐→开发↔测试→产品验收→发布/回流时使用。
---

# 四 Agent 协作（四 Agent 协作）
...
1. **启动一次运行**  
   ...
   - 读取 `workspace/four-roles-core/flow.json` 与 `workspace/four-roles-core/roles/*.md` 中当前阶段对应 Agent 说明。

2. **每一轮（当前阶段）**  
   - 读取 `workspace/four-roles-core/context.json`，得到当前 `phase` 与已有 `outputs`。
   - 根据 `flow.json` 的 `roleByPhase[phase]` 确定当前 Agent **role**（pm / architect / dev / qa / all）。
   - **若 role 为 pm、architect、dev、qa 之一**：使用工具 **sessions_send**，**sessionKey** 为 `agent:<role>:main`（例如 `agent:pm:main`），**message** 包含：当前 phase、用户 instruction、context 摘要（含已有 outputs 与 attachmentRefs）、以及「请以你 Agent 身份输出本阶段产出，开头标明【Agent 名】」。**timeoutSeconds** 建议 300。将返回的 **reply** 作为本阶段产出，写入 `context.outputs[phase]`（或 dev_N/test_N，见下），并更新 `context.phase`。
...
3. **流程顺序**（与《四Agent协作-完整需求文档.md》一致）  
...
- 每步向用户展示时明确标注当前**阶段**与**执行 Agent**（pm/architect/dev/qa），便于验收 A1、A2。
- 通过 sessions_send 获得的 reply 即该 Agent 产出；accept 阶段须根据 pm 的 reply 解析出通过/不通过并更新 context.acceptResult。
...
- 完整需求与验收标准：`四Agent协作-完整需求文档.md`（仓库根或工作区上级）
